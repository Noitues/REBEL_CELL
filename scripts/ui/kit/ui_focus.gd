...
static func focus_first(root: Node, only_if_lost: bool = false) -> void:
	if root == null:
		return
	_focus_now.call_deferred(root, only_if_lost)


## Untyped on purpose: the deferred call can land after the panel was freed.
static func _focus_now(target, only_if_lost: bool = false) -> void:
	...
	var usable := owner != null and is_instance_valid(owner) and owner.is_visible_in_tree() and not (owner is BaseButton and (owner as BaseButton).disabled)
